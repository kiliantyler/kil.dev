...
function buildThemeScript(): string {
  ...
  return `("use strict");(function(){try{var cfg={base:${JSON.stringify(base)},seasonal:${JSON.stringify(seasonal)}};function inRange(dt,s,e){var y=dt.getFullYear(),m=dt.getMonth()+1,d=dt.getDate();var crosses=(e.m<s.m)||((e.m===s.m)&&(e.d<=s.d));var onOrAfterStart=(m>s.m)||((m===s.m)&&(d>=s.d));var sy=crosses?(onOrAfterStart?y:y-1):y;var ey=crosses?sy+1:sy;var sd=new Date(sy,s.m-1,s.d);var ed=new Date(ey,e.m-1,e.d);return dt>=sd&&dt<ed}var now=new Date();var active=cfg.seasonal.filter(function(s){return inRange(now,s.start,s.end)});var allowed=cfg.base.concat(active.map(function(s){return s.theme}));var defaultTheme=(active[0]&&active[0].theme)||null;function isAllowed(t){return !!t&&allowed.indexOf(t)!==-1}function getCookieTheme(){try{var m=document.cookie.match(/(?:^|; )theme=([^;]+)/);return m?decodeURIComponent(m[1]):null}catch(_){return null}}function getLsTheme(){try{return localStorage.getItem('theme')}catch(_){return null}}var cookieTheme=getCookieTheme();var lsTheme=getLsTheme();var sysDark=(window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches);var pref=(lsTheme||cookieTheme||'system');var baseClass=sysDark?'dark':'light';var explicit=null;var overlay=null;if(pref==='system'){overlay=defaultTheme||null}else if(pref!=='system'&&isAllowed(pref)){explicit=pref}else if(defaultTheme){explicit=defaultTheme}else{explicit=baseClass}var root=document.documentElement;var disable=document.createElement('style');disable.id='__disable-theme-transitions';disable.appendChild(document.createTextNode('*,*::before,*::after{transition:none !important;animation:none !important}'));try{document.head.appendChild(disable)}catch(_){}var targetClasses=(function(){var t=[];if(explicit){t.push(explicit)}else{t.push(baseClass);if(overlay){t.push(overlay)}}return t})();var known=(function(){var set={};function add(a){for(var i=0;i<a.length;i++){set[a[i]]=1}}add(cfg.base);add(active.map(function(s){return s.theme}));add(['light','dark']);return Object.keys(set)})();for(var i=0;i<known.length;i++){var cls=known[i];if(targetClasses.indexOf(cls)===-1){try{root.classList.remove(cls)}catch(_){}}}for(var j=0;j<targetClasses.length;j++){try{root.classList.add(targetClasses[j])}catch(_){}}try{root.dataset.themePref=(pref||'');root.dataset.seasonalDefault=(overlay||'');root.dataset.appliedTheme=(explicit||baseClass||'')}catch(_){}}catch(e){}finally{try{var rm=function(){var el=document.getElementById('__disable-theme-transitions');if(el&&el.parentNode){el.parentNode.removeChild(el)}};if('requestAnimationFrame'in window){requestAnimationFrame(function(){rm()})}else{setTimeout(rm,0)}}catch(_){}}})();`
}

export const buildInitThemeScript = buildThemeScript
export const buildPreThemeScript = buildThemeScript
